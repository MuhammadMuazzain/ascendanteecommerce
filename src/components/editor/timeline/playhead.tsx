...
import { seekEditor, stopEditorPlayback } from "@/lib/editor-playback";
...
const Playhead = ({ scrollLeft, scale }: { scrollLeft: number; scale: ITimelineScaleState }) => {
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent | TouchEvent | any) => {
      ...
      if (projectStore.getState().isPlaying && !dragRef.current.stoppedPlayback) {
        stopEditorPlayback();
        dragRef.current.stoppedPlayback = true;
      }
  ...
  const handleMouseUp = useCallback(() => {
    if (dragRef.current.isDragging) {
      const finalTimeUs = localTimeUs ?? projectStore.getState().currentTime;
      dragRef.current.isDragging = false;
      dragRef.current.stoppedPlayback = false;
      setLocalTimeUs(null);
      void seekEditor(finalTimeUs);
