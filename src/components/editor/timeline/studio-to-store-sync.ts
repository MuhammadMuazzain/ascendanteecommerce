 ...
 */
/** Prevents Core ↔ Studio play/pause feedback loops (stack overflow). */
let playbackSyncGuard = false;

export const addStudioSync = (studio: Studio, timeline: CanvasTimeline): (() => void) => {
  // Mirror Studio transport state to the timeline store (UI playhead / play button only).
  // Do not call core.play() / core.pause() here — StudioBridge already does Studio sync from Core.
  const syncPlayingFromStudio = (payload?: { isPlaying?: boolean }) => {
    if (playbackSyncGuard) return;

    const nextPlaying = Boolean(payload?.isPlaying);
    if (projectStore.getState().isPlaying === nextPlaying) return;

    playbackSyncGuard = true;
    try {
      projectStore.getState().setIsPlaying(nextPlaying);
    } finally {
      playbackSyncGuard = false;
    }
  };

  studio.on("play", syncPlayingFromStudio);
  studio.on("pause", syncPlayingFromStudio);
  ...
  return () => {
    studio.off("play", syncPlayingFromStudio);
    studio.off("pause", syncPlayingFromStudio);
